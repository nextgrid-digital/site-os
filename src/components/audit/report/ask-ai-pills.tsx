...
function GeminiLogo({ className, gradientId }: { className?: string; gradientId: string }) {
  return (
    <svg viewBox="0 0 24 24" className={className} aria-hidden xmlns="http://www.w3.org/2000/svg">
      <defs>
...
function ProviderLogo({
  ...
  switch (id) {
    ...
    case 'gemini':
      return <GeminiLogo className={className} gradientId={geminiGradientId} />;
    default: {
...
export function AskAiPills({ askAi, question, className = '' }: AskAiPillsProps) {
  const geminiGradientId = `gemini-${useId().replace(/:/g, '')}`;
        ...
        {links.map((link) => (
          ...
          >
            <ProviderLogo
              ...
              geminiGradientId={geminiGradientId}
            />
