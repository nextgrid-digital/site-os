    ...
    <li data-cid={cids[0]} className={cn("list-item", styles.className)}>
      <div data-cid={cids[1]} className={cn("block relative rounded-lg overflow-hidden w-full", styles.className2)}>
        ...
        <div data-cid={cids[3]} className={cn("w-121.5 flex relative z-10 py-2 px-2.5 justify-between items-center gap-3 max-md:w-[17.8125rem] max-md:px-2 md:max-lg:w-77.5", styles.className4)}>
          <span data-cid={cids[4]} className={cn("w-99.5 block min-w-0 flex-1 overflow-hidden text-xs leading-4 whitespace-nowrap text-nowrap max-md:w-[13.0625rem] md:max-lg:w-55.5", styles.className5)} title={d.label}>
            {d.text}
          </span>
          <div data-cid={cids[5]} className="flex justify-end items-baseline shrink-0 gap-4 max-md:gap-2">
            <p data-cid={cids[6]} className="block text-muted-foreground text-[0.6875rem] leading-[0.6875rem] text-right w-14 max-md:w-12">
              {d.description}
            </p>
          </div>
        </div>
